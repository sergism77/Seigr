...
def rollback_to_previous_state(
    ...
    if not validate_acl_for_integrity_check(seigr_file.acl, user_id):
        logger.warning(
            f"User {user_id} lacks permission for rollback on segment {seigr_file.hash}."
        )
        return False
    ...
    try:
        ...
        logger.info(
            f"Reverted segment {seigr_file.hash} data to previous layer at timestamp {previous_layer.timestamp}."
        )
