...
def get_activity_log():
    ...
    if not os.path.exists(Config.PING_LOG_PATH):
        response = OperationLog(
            ...
            status="error",
            details="No activity log found"
        )
        return Response(response.SerializeToString(), content_type='application/octet-stream', status=404)
    ...
    try:
        ...
        return Response(response.SerializeToString(), content_type='application/octet-stream', status=500)
