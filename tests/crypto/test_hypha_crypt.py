...
def hypha_crypt():
    ...
    return HyphaCrypt(data=SAMPLE_DATA, segment_id=SEGMENT_ID, hash_depth=HASH_DEPTH, use_senary=USE_SENARY)
...
def test_layered_hash_tree_generation(hypha_crypt):
    ...
    for depth in range(1, HASH_DEPTH + 1):
        ...
        assert isinstance(hash_tree[layer_key], list), f"{layer_key} should be a list of hashes"
...
def test_log_integrity_verification(hypha_crypt):
    ...
    verification_entry = hypha_crypt.log_integrity_verification(status, verifier_id, integrity_level, details)
