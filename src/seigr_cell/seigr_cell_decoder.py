        ...
        try:
            ...
            if success:
                ...
                secure_logger.log_audit_event(
                    severity=1,
                    category="Integrity",
                    message=f"Integrity verified for Seigr Cell segment {self.segment_id}",
                    sensitive=False,
                )
            else:
                logger.warning(
                ...
                secure_logger.log_audit_event(
                    severity=2,
                    category="Integrity",
                    message=f"Integrity verification failed for Seigr Cell segment {self.segment_id}",
                    sensitive=True,
                )
            return success
        except Exception as e:
            ...
            secure_logger.log_audit_event(
                severity=4,
                category="Integrity",
                message=f"Integrity verification error for segment {self.segment_id}: {e}",
                sensitive=True,
            )
