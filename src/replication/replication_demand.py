...
class DemandBasedReplication:
    ...
    def adapt_based_on_demand(
        ...
        if access_count < demand_threshold:
            logger.info(
                f"Segment {segment.segment_hash} access below threshold ({access_count}/{demand_threshold}). No replication needed."
            )
            return False
        ...
        try:
            success = self.replication_manager.replicate_segment(
                segment.segment_hash, new_replication_count
            )
            if success:
                logger.info(
                    f"Demand-based replication completed for segment {segment.segment_hash} with replication count: {new_replication_count}"
                )
                return True
            else:
                raise ValueError(
                    f"Replication failed for segment {segment.segment_hash}. Requested count: {new_replication_count}"
                )
    ...
    def calculate_demand_scale(self, access_count: int, min_replication: int) -> int:
        ...
        logger.debug(
            f"Calculated demand-based replication count for access {access_count}: {replication_count}"
        )
        return replication_count

    def monitor_and_replicate_by_demand(
        self, segments_status: dict, demand_threshold: int, min_replication: int
    ):
        """
        ...
        for segment_hash, status in segments_status.items():
            ...
            if not segment_metadata:
                logger.warning(
                    f"Missing metadata for segment {segment_hash}. Skipping demand-based replication check."
                )
                continue

            if access_count >= demand_threshold:
                try:
                    self.adapt_based_on_demand(
                        segment_metadata,
                        access_count,
                        demand_threshold,
                        min_replication,
