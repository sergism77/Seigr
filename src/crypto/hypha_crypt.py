...
class HyphaCrypt:
    ...
    def generate_encryption_key(self, password: str = None) -> bytes:
        ...
        try:
            ...
            logger.debug(f"{SEIGR_CELL_ID_PREFIX} Generated encryption key for segment {self.segment_id}")
            return key
    ...
    def compute_primary_hash(self):
        ...
        try:
            ...
            self._log_error(f"{SEIGR_CELL_ID_PREFIX}_primary_hash_fail", "Failed to compute primary hash", e)
            raise
        ...
        for depth in range(1, self.hash_depth + 1):
            ...
            for item in current_layer:
                ...
                try:
                    ...
                    self._log_error(f"{SEIGR_CELL_ID_PREFIX}_layer_hash_fail", f"Failed to compute layer {depth} hash", e)
                    raise
            self.tree[f"Layer_{depth}"] = next_layer
            current_layer = next_layer

        logger.info(f"{SEIGR_CELL_ID_PREFIX} Layered hashes computed for segment {self.segment_id} up to depth {self.hash_depth}")
        return self.tree
    ...
    def _log_layer_event(self, depth, layer_hash):
        ...
        logger.debug(f"{SEIGR_CELL_ID_PREFIX} Layer event logged at depth {depth} for segment {self.segment_id}")
    ...
    def verify_integrity(self, reference_tree, partial_depth=None):
        ...
        for depth in range(1, partial_depth + 1):
            ...
            if generated_layer != reference_layer:
                ...
                logger.warning(f"{SEIGR_CELL_ID_PREFIX} Integrity check failed at depth {depth} for segment {self.segment_id}")

        logger.info(f"{SEIGR_CELL_ID_PREFIX} Integrity verification for segment {self.segment_id}: {verification_results['status']}")
