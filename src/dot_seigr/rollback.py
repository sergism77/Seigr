...
def revert_segment_data(seigr_file: SeigrFile, previous_layer: dict):
    """
    ...
    Args:
        ...
        previous_layer (dict): Dictionary containing the data snapshot of the previous state.
    """
    # Restore main data and metadata from the previous state
    seigr_file.data = previous_layer["data_snapshot"]["data"]
    seigr_file.hash = previous_layer["data_snapshot"]["hash"]  # Update hash to match previous state
    seigr_file.primary_link = previous_layer["data_snapshot"].get("primary_link")
    seigr_file.secondary_links = previous_layer["data_snapshot"].get("secondary_links", [])
    seigr_file.coordinate_index = previous_layer["data_snapshot"].get("coordinate_index")
    
    # Refresh the temporal layer to reflect the reverted state
    seigr_file.add_temporal_layer()

    logger.debug(f"Segment {seigr_file.hash} reverted to previous state with hash {previous_layer['data_snapshot']['hash']}.")
