...
def hash_to_protobuf(
    ...
    logger.debug(
        f"{SEIGR_CELL_ID_PREFIX} Generated protocol buffer HashData: {hash_data}"
    )
    return hash_data
...
def verify_hash(data: bytes, expected_hash: str, salt: str = None) -> bool:
    ...
    try:
        ...
        logger.debug(
            f"{SEIGR_CELL_ID_PREFIX} Hash verification result: {'Match' if match else 'No Match'} for hash: {actual_hash}"
        )
        return match
...
def protobuf_verify_hash(
    ...
    logger.debug(
        f"{SEIGR_CELL_ID_PREFIX} Protobuf hash verification status: {status.name} for hash ID: {protobuf_hash.hash_id}"
    )
