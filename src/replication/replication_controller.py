...
class ReplicationController:
    def __init__(self, min_replication: int, demand_threshold: int, network_hyphens: list):
        ...
        logger.info(
            "ReplicationController initialized with min_replication=%d, demand_threshold=%d, hyphens=%s",
            min_replication,
            demand_threshold,
            network_hyphens,
        )
    ...
    def monitor_and_adapt_replication(self, segments_status: dict):
        ...
        for segment_hash, status in segments_status.items():
            ...
            if not isinstance(segment_metadata, SegmentMetadata):
                logger.warning(
                    f"Skipping replication adaptation for segment {segment_hash}: Invalid or missing metadata."
                )
                continue
            ...
            logger.debug(
                f"Monitoring segment {segment_hash}: Access={access_count}, Threat={threat_level}, "
    ...
    def _handle_demand_replication(self, segment_metadata: SegmentMetadata, access_count: int):
        ...
        if access_count > self.demand_threshold:
            logger.info(
                f"High demand detected for segment {segment_metadata.segment_hash} with access count {access_count}."
            )
            try:
    ...
    def _handle_threat_replication(self, segment_metadata: SegmentMetadata, threat_level: int):
        ...
        if threat_level > 0:
            logger.info(
                f"Threat-based replication triggered for segment {segment_metadata.segment_hash} with threat level {threat_level}."
            )
            try:
                self.threat_replicator.adaptive_threat_replication(
                    segment_metadata, threat_level, self.min_replication
                )
            except Exception as e:
                logger.error(
                    f"Threat replication failed for segment {segment_metadata.segment_hash}: {e}"
                )
    ...
    def _handle_self_healing(
        ...
        if network_replication < self.min_replication:
            logger.info(
                f"Self-healing required for segment {segment_metadata.segment_hash}. Current replication is below minimum."
            )
            try:
                ...
                logger.error(
                    f"Self-healing replication failed for segment {segment_metadata.segment_hash}: {e}"
                )
