...
class TestImmuneSystem(TestCase):
    ...
    def setUp(self):
        ...
        self.immune_system.threat_detector.threat_log = []  # Ensure clean threat log before each test
    ...
    def test_immune_ping_fails_with_corrupted_data(self, mock_verify_integrity):
        ...
        self.assertEqual(len(self.immune_system.threat_detector.threat_log), 1, "Expected a single threat to be logged")
        self.assertEqual(
    ...
    @mock.patch('src.immune_system.rollback_handling.RollbackHandler.verify_rollback_availability', return_value=True)
    def test_rollback_segment_invokes_rollback_to_previous_state(self, mock_verify_rollback_availability, mock_rollback):
        ...
        self.assertEqual(mock_rollback.call_args[0][0], seigr_file, "Expected rollback_to_previous_state to be called with the correct seigr_file.")

    @mock.patch('src.immune_system.rollback_handling.rollback_to_previous_state')
    @mock.patch('src.immune_system.rollback_handling.RollbackHandler.verify_rollback_availability', return_value=False)
