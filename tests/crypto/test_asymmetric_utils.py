...
def test_generate_key_pair():
    ...
    ), "Public key should be in bytes format."
    assert isinstance(
        key_pair.private_key, bytes
    ), "Private key should be in bytes format."
...
def test_sign_and_verify_signature():
    ...
    ), "SignatureLog should contain a 'signature' field."
    assert isinstance(
        signature_log.signature, bytes
    ), "Signature should be in bytes format."
    ...
    is_valid_tampered = verify_signature(
        tampered_data, signature_log.signature, key_pair.public_key
    )
    assert (
        not is_valid_tampered
    ), "Signature verification should fail with tampered data."
...
def test_serialize_and_load_public_key():
    ...
    assert (
        serialized_public_key == key_pair.public_key
    ), "Serialized public key should match the original."
    assert (
        reloaded_public_key.public_numbers() == public_key.public_numbers()
    ), "Reloaded public key should match."
...
def test_serialize_and_load_private_key():
    ...
    assert (
        serialized_private_key == key_pair.private_key
    ), "Serialized private key should match the original."
    assert (
        reloaded_private_key.private_numbers() == private_key.private_numbers()
    ), "Reloaded private key should match."
...
def test_invalid_key_loading():
    ...
    assert "Failed to load RSA public key" in str(
        excinfo.value
    ), "Expected a ValueError indicating public key loading failure."

    # Test loading an invalid private key
    with pytest.raises(ValueError) as excinfo:
        load_private_key(invalid_pem)
    assert "Failed to load RSA private key" in str(
        excinfo.value
    ), "Expected a ValueError indicating private key loading failure."
...
def test_sign_data_with_invalid_key():
    ...
    assert "Failed to load RSA private key" in str(
        excinfo.value
    ), "Expected a ValueError for invalid private key signing."
...
def test_error_logging_for_invalid_key():
    ...
    try:
        ...
        assert (
            error_log.severity == ErrorSeverity.ERROR_SEVERITY_HIGH
        ), "Error severity should be high for invalid key."
        assert (
            error_log.resolution_strategy
            == ErrorResolutionStrategy.ERROR_STRATEGY_LOG_AND_CONTINUE
