...
def log_secure_action(action: str, metadata: dict = None, sensitive: bool = False, use_senary: bool = False):
    ...
    return _secure_logger_instance.log_audit_event(severity, category, message, sensitive=sensitive, use_senary=use_senary)
...
class SecureLogger:
    ...
    def log_audit_event(self, severity: LogLevel, category: LogCategory, message: str, sensitive: bool = False, use_senary: bool = False) -> AuditLogEntry:
        try:
            ...
            logger.info(log_message + (" (Encrypted and Senary Encoded)" if use_senary and sensitive else " (Encrypted)" if sensitive else ""))
            return audit_entry
        except Exception as e:
