...
def test_verify_integrity_valid():
    ...
    assert verify_integrity(data, correct_hash) is True, "Expected valid integrity check to return True"

def test_verify_integrity_invalid():
    data = b"test data"
    incorrect_hash = "incorrecthash12345"
    assert verify_integrity(data, incorrect_hash) is False, "Expected invalid integrity check to return False"

def test_verify_integrity_empty_data():
    data = b""
    empty_hash = hashlib.sha256(data).hexdigest()
    assert verify_integrity(data, empty_hash) is True, "Expected empty data with correct hash to pass integrity check"
...
def test_verify_hierarchical_integrity_invalid():
    ...
    assert verify_hierarchical_integrity(data, hash_tree) is False, "Expected invalid hierarchical integrity check to fail"

def test_verify_hierarchical_integrity_empty_tree():
    data = b"test data"
    hash_tree = {}
    assert verify_hierarchical_integrity(data, hash_tree) is False, "Expected empty hash tree to fail integrity check"
